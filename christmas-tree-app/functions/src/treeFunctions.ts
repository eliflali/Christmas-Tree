...
export const createTree = onCall(async (request: CallableRequest) => {
  const { userId, treeName } = request.data;
  ...
  try {
    ...
    return { success: true, treeId: treeDoc.id, message: 'Tree created successfully.' };
  } catch (error) {
...
export const deleteTree = onCall(async (request: CallableRequest) => {
  ...
  try {
    ...
    const notesSnapshot = await db.collection('trees').doc(treeId).collection('notes').get();
